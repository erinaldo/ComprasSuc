...
#End Region
   ...
   Private Sub frmAjuste_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Activated
      Call StatusFormDisplay(Me.Name)
   End Sub

   Private Sub frmAjuste_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
      Dim oCursor As Cursor
      oCursor = Me.Cursor
      Me.Cursor = Cursors.WaitCursor
   ...
   Private Sub grdMainLoad()
      Try
         With moAjuste
            .SelectFilter = clsAjuste.SelectFilters.Grid
            .WhereFilter = clsAjuste.WhereFilters.Grid
            .OrderByFilter = clsAjuste.OrderByFilters.Grid
            ...
            If .Open() Then
               ...
               For Each oRow As DataRow In .DataSet.Tables(.TableName).Rows
                  If (tblComprasNros.Count > 0) Then
                     oRow("CompraNro") = ToStr(tblComprasNros.Item(ToLong(oRow("AjusteId"))))
                  End If

                  If (tblCreditosNros.Count > 0) Then
                     oRow("CreditoNro") = ToStr(tblCreditosNros.Item(ToLong(oRow("AjusteId"))))
                  End If
               Next
   ...
   Private Sub DataShow()
      ...
      Try
         If (grdMain.GetRow.Selected) And (ToLong(grdMain.GetValue("AjusteId")) > 0) Then
            With oAjuste
               .AjusteId = ToLong(grdMain.GetValue("AjusteId"))
   ...
   Private Sub FormNew()
      ...
      Try
         With frm
            ...
            oAjuste.EmpresaId = mlngEmpresaId
   ...
   Private Sub DataEdit()
      ...
      Try
         If grdMain.RowCount > 0 Then
            If ToLong(grdMain.GetValue("AjusteId")) > 0 Then
               If ToLong(grdMain.GetValue("EstadoId")) <> 12 Then
                  ...
                  End With
               Else
                  MessageBox.Show("Imposible Editar el Ajuste", Me.Text, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
               End If
            Else
         ...
         If grdMain.RowCount > 0 Then
            If ToLong(grdMain.GetValue("AjusteId")) > 0 Then
               If MessageBox.Show("¿Realmente quiere eliminar el registro?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                  With oAjuste
                     .WhereFilter = clsAjuste.WhereFilters.PrimaryKey
                     .AjusteId = ToLong(grdMain.GetValue("AjusteId"))
   ...
   Private Function getNrosCompras(ByRef tblCreditosNros As Hashtable) As Hashtable
      ...
      With oAjusteDet
         .SelectFilter = clsAjusteDet.SelectFilters.NRO_CREDITO_COMPRA
         .WhereFilter = clsAjusteDet.WhereFilters.NRO_CREDITO_COMPRA
         ...
         If (.Open) Then
            For Each oRow As DataRow In .DataSet.Tables(.TableName).Rows
               ...
               If ToLong(oRow("CompraId")) > 0 Then
                  ...
                  If ((tblAjuste.Contains(ToLong(oRow("AjusteId"))))) Then

                     tblCompra = tblAjuste.Item(ToLong(oRow("AjusteId")))
                     If Not (tblCompra.Contains(ToLong(oRow("CompraId")))) Then
                        tblCompra.Add(ToLong(oRow("CompraId")), oRow("CompraNro"))
                        nros = ToStr(tblComprasNros.Item(ToLong(oRow("AjusteId")))) + "; " + ToStr(oRow("CompraNro"))
                        tblComprasNros.Item(ToLong(oRow("AjusteId"))) = nros
                     End If
                  Else
                     tblCompra = New Hashtable
                     tblCompra.Add(ToLong(oRow("CompraId")), oRow("CompraNro"))
                     tblAjuste.Add(ToLong(oRow("AjusteId")), tblCompra)
                     tblComprasNros.Add(ToLong(oRow("AjusteId")), ToLong(oRow("CompraNro")))
                  End If
               End If

               If ToLong(oRow("CreditoId")) > 0 Then

                  If ((tblAjusteCred.Contains(ToLong(oRow("AjusteId"))))) Then

                     tblCredito = tblAjusteCred.Item(ToLong(oRow("AjusteId")))
                     If Not (tblCredito.Contains(ToLong(oRow("CreditoId")))) Then
                        tblCredito.Add(ToLong(oRow("CreditoId")), oRow("CreditoNro"))
                        nros = ToStr(tblCreditosNros.Item(ToLong(oRow("AjusteId")))) + "; " + ToStr(oRow("CreditoNro"))
                        tblCreditosNros.Item(ToLong(oRow("AjusteId"))) = nros
                     End If
                  Else
                     tblCredito = New Hashtable
                     tblCredito.Add(ToLong(oRow("CreditoId")), oRow("CreditoNro"))
                     tblAjusteCred.Add(ToLong(oRow("AjusteId")), tblCredito)
                     tblCreditosNros.Add(ToLong(oRow("AjusteId")), ToLong(oRow("CreditoNro")))
                  End If
   ...
   Private Sub PrintReport()
      Dim rpt As New rptAjuste

      rpt.DataSource = GetDataView(moAjuste.DataSet.Tables(moAjuste.TableName))

      rpt.lblTitle.Text = "Historial de Ajustes"
      rpt.Document.Name = "Historial de Ajustes"
   ...
   Private Sub grdMainFindRow(ByVal lngID As Long)
      ...
      oCol = grdMain.RootTable.Columns("AjusteId")
   ...
   Private Sub FormInit()
      ...
      moAjuste = New clsAjuste(clsAppInfo.ConnectString)

      txtMaxReg.Text = ToStr(CompraParaRowsMaxFind(clsAppInfo.EmpresaId, clsAppInfo.GestionId, 10, clsAppInfo.SucursalId))
   End Sub

   Private Sub grdMainInit()
      With grdMain
         ebrMain.Groups(0).Text = "Historial de Ajustes"
         .ContextMenu = mnuMain
         ...
         .RootTable.Columns("AjusteNro").Caption = "Número"
         .RootTable.Columns("AjusteNro").Width = 100
         .RootTable.Columns("AjusteNro").CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center
         .RootTable.Columns("AjusteNro").HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center
         ...
         .RootTable.Columns("AjusteDes").Caption = "Descripción"
         .RootTable.Columns("AjusteDes").Width = 100
         .RootTable.Columns("AjusteDes").CellStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Near
         .RootTable.Columns("AjusteDes").HeaderStyle.TextAlignment = Janus.Windows.GridEX.TextAlignment.Center
   ...
   Private Sub frmAjuste_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
   ...
   Private Sub frmAjuste_Closed(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Closed
      moAjuste.Dispose()
      Call ClearMemory()
   End Sub
